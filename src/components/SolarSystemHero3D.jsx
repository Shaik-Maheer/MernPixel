  ...
  return (
    <>
      ...
      <points ref={emberRef}>
        ...
        <pointsMaterial color="#B304B5" size={0.15} opacity={0.4} transparent sizeAttenuation blending={THREE.AdditiveBlending} />
      </points>
      {/* Drei built-in extras */}
      <Stars radius={50} depth={50} count={12000} factor={8} saturation={1} fade speed={3} />
      <Sparkles count={2000} scale={30} size={15} speed={0.4} opacity={0.6} color="#6E026F" />
    </>
  )
}
  ...
  return (
    <group ref={groupRef}>
      <mesh>
        ...
        <meshStandardMaterial color="#FFFFFF" emissive="#000000" emissiveIntensity={4} wireframe={false} />
      </mesh>
      <mesh>
        <sphereGeometry args={[3.5, 32, 32]} />
        <meshBasicMaterial color="#B304B5" wireframe transparent opacity={0.4} blending={THREE.AdditiveBlending} />
      </mesh>
      <mesh>
        <sphereGeometry args={[3.8, 48, 48]} />
        <meshBasicMaterial color="#6E026F" wireframe transparent opacity={0.2} blending={THREE.AdditiveBlending} />
      </mesh>
    </group>
  )
}
    ...
    <group ref={ringGroup}>
      {Array.from({length: 12}).map((_, i) => (
        <mesh key={i} rotation={[Math.PI / 2 + (i*0.1), i*0.2, 0]}>
          ...
          <meshBasicMaterial color={i % 2 === 0 ? "#B304B5" : "#000000"} transparent opacity={0.3} blending={THREE.AdditiveBlending} />
        </mesh>
      ))}
    </group>
  )
...
function Scene() {
  return (
    <>
      ...
      <pointLight position={[0, 0, 0]} intensity={10} color="#000000" distance={30} />
      <StarField />
      <FireSun />
      <OrbitRings />
      {[...Array(25)].map((_, i) => (
        <ChaosPlanet 
          ...
          color={pseudoRandom(i, 7) > 0.5 ? "#000000" : "#6E026F"} 
